def read_spisok_first(a): # Функция собирает данные из файла
    with open(a, 'r') as f:    # Нежно читаем файл с  обязательным закрытия оного
        ...
        data = data.replace('\t', ",").replace('\n', '').upper().split(',') # Легкое порно: меняем знаки табуляции запятыми, а знаки переносов убираем вовсе. Все буквы делаем заглавными. Назначаем запятую разделителем всего этого боголепия, что незамедлительно превращает данные в список.
        return data
